...
date: "2024-11-11"
bibliography: AgslPredict_refs.bib
---
...
The raw data used for relative abundance are available at <https://vectorbase.org/common/downloads/popbio-map-legacy/VectorBase-popbio-map-rel64-July2023-Samples.csv.gz>. The R script that downloads and processes these data for analysis is available in this package by executing
```{r ra, eval = FALSE}
...
This script reads in the relative abundance data and covariates. A series of multinomial logit GLMs are then fit using a backward stepwise BIC procedure on two groups of models:

1. 1 month windowed meteorological covariates without transformations;
2. 1 month windowed meteorological covariates, with log-one-plus transformation of all nonnegative covariates.

The best fitting model was found in group 1). The best fitting model included all possible covariates and most interactions and squared terms. The directory includes spatial plots of residuals by species, model predictions by species assuming most recent survey year and observed data by species. 
...
The predictions were processed on high performance computing by the following scripts.
